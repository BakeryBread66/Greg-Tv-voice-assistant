...
export function initClone(config) {
  ...
  if (!fs.existsSync(python)) {
    state = "unavailable";
    info = { reason: `no Python at ${path.relative(ROOT, python)} — run the clone setup first` };
    return Promise.resolve(false);
  }
  if (!fs.existsSync(reference)) {
    state = "unavailable";
    info = { reason: `no reference clip at ${path.relative(ROOT, reference)}` };
    return Promise.resolve(false);
  }
  ...
  const problem = referenceProblem(describeWav(readHead(reference), { fileSize: fs.statSync(reference).size }));
  if (problem) {
    state = "unavailable";
