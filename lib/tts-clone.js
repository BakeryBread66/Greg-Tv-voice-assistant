...
import { clonePython, setupCommand } from "./platform.js";
...
// `Scripts/python.exe` on Windows, `.venv-clone/bin/python` everywhere else.
const DEFAULT_PYTHON = clonePython();
...
export function initClone(config) {
  ...
  if (!fs.existsSync(python)) {
    ...
    info = {
      reason:
        ...
        `Run: ${setupCommand("-Clone")}  (about 6 GB; any Python from 3.10 to 3.13)`,
    };
    return Promise.resolve(false);
  }
  ...
  return new Promise((resolve) => {
    try {
      child = spawn(python, [SCRIPT], {
        ...
        env: {
          ...
          ...(settings.precision != null ? { GREG_CLONE_PRECISION: String(settings.precision) } : {}),
    ...
    child.stdout.on("data", (chunk) => {
      ...
      const match = /READY reference=(\S+) rate=(\d+) precision=(\S+)/.exec(line);
      if (match) {
        clearTimeout(timer);
        state = "ready";
        ...
        //
        // Precision comes off the READY line rather than from `settings`, because
        // the sidecar is the one that resolves it — "auto" means fp16 on CUDA and
        // fp32 on the CPU, and a config asking for fp16 on a CPU run gets fp32.
        // Reading it back means the cache key describes what was actually loaded.
        info = {
          reference: match[1],
        ...
        console.log(`[voice] cloned voice ready (${match[1]}, ${match[3]})`);
        resolve(true);
      }
    });
