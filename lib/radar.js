...
export function describeRadar(radar) {
  if (!radar?.covered) return "There is no weather radar for this area.";
  if (!radar.frames?.length) return "The radar hasn't come back yet.";
  return `Radar for ${radar.place || "your area"}, from station ${radar.station}, covering the last twenty minutes.`;
}
