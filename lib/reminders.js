...
let file = path.join(ROOT, "reminders.json");
...
/**
 * Point the store at a different file, disarming everything first. Only the
 * tests call it: this module wrote to one hard-coded reminders.json, so nothing
 * that edits a reminder could be tested without editing the user's — and this
 * project already has a fake daily medicine reminder in its history from exactly
 * that. Call initReminders afterwards to load the new file.
 */
export function useRemindersFile(target) {
...
/** Cancel exactly one, by id. The Settings dialog's delete: no guessing from words. */
export function deleteReminder(id) {
  const item = items.find((i) => i.id === id);
  if (!item) return { error: "That reminder isn't there any more — it may already have gone off." };
  clearTimeout(handles.get(id));
  handles.delete(id);
  items = items.filter((i) => i !== item);
  save();
  return { ok: true, removed: item.text };
}

/**
 * Change a reminder from the Settings dialog: what it says, the time of day it
 * goes off, and whether it repeats.
 *
 * A timer can only be renamed. It counts down from when it was set, and there is
 * no time of day in it to change.
 *
 * A one-off keeps its DAY. Moving "call the dentist, Friday 3pm" to 4pm must not
 * make it today at 4pm, which is what parsing the new time as "the next 4pm"
 * would do. If that leaves it in the past, it is refused rather than fired at
 * once or quietly moved to tomorrow.
 *
 * @param {string} id
 * @param {{ text?: string, time?: string, repeat?: "none"|"daily"|"weekdays" }} change
 *        `time` is "HH:MM", 24-hour, as an <input type="time"> gives it.
 */
export function updateReminder(id, { text, time, repeat } = {}, now = Date.now()) {
  const item = items.find((i) => i.id === id);
  if (!item) return { error: "That reminder isn't there any more — it may already have gone off." };

  const wording = text === undefined ? item.text : String(text).trim().replace(/\s+/g, " ");
  if (!wording) return { error: "A reminder needs something to say." };
  if (wording.length > 200) return { error: "That's too long for a reminder — keep it under 200 characters." };

  let dueAt = item.dueAt;
  let repeats = item.repeat ?? null;
  if (item.kind !== "timer") {
    if (repeat !== undefined) {
      if (!["none", "daily", "weekdays"].includes(repeat)) return { error: `A reminder repeats daily, on weekdays, or not at all — not "${repeat}".` };
      repeats = repeat === "none" ? null : repeat;
    }
    if (time !== undefined && time !== "") {
      const match = /^(\d{1,2}):(\d{2})$/.exec(String(time));
      if (!match || Number(match[1]) > 23 || Number(match[2]) > 59) return { error: `"${time}" isn't a time of day.` };
      // A repeating one counts from today: its next occurrence may be tomorrow,
      // and a later time tonight must not be skipped because of that.
      const moved = new Date(repeats ? now : dueAt);
      moved.setHours(Number(match[1]), Number(match[2]), 0, 0);
      dueAt = moved.getTime();
    }
    if (repeats) {
      // The next time it is due, and never on a weekend for "weekdays".
      dueAt = rollForward(dueAt, repeats, now);
      if (repeats === "weekdays" && WEEKEND.has(new Date(dueAt).getDay())) dueAt = rollForward(dueAt, "weekdays", dueAt);
    } else if (dueAt <= now) {
      return { error: "That time has already passed. Pick a later one, or make it repeat." };
    }
  } else if ((time !== undefined && time !== "") || (repeat !== undefined && repeat !== "none")) {
    return { error: "A timer counts down from when it was set, so only its name can change." };
  }

  item.text = wording;
  item.dueAt = dueAt;
  if (repeats) item.repeat = repeats;
  else delete item.repeat;
  clearTimeout(handles.get(id));
  arm(item);
  save();
  return { ok: true, reminder: listReminders().find((i) => i.id === id) };
}
