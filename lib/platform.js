...
/**
 * Which Python the Whisper and Piper sidecars are spawned with, when config does
 * not name one.
 *
 * This is the ears-and-voice half of "does Greg run on Linux", and until now the
 * default was the Windows launcher `py` for everyone — so on Linux the two
 * sidecars spawned a command that does not exist, failed with ENOENT, and Greg
 * fell back to the cloud voice and browser speech with nothing saying why. The
 * docs claimed both worked; the default said otherwise, and the default is the
 * authority.
 *
 * On Windows the launcher is `py` (callers add `-3`). Everywhere else a system
 * `pip install` is refused by PEP 668, so setup-greg.sh puts the sidecars in a
 * `.venv` at the repo root — the same "a folder is the install" shape the cloned
 * voice already uses. Prefer that venv when it is there, and fall back to
 * `python3` for someone who installed the packages another way.
 *
 * Pure: platform, root and the existence check are all injected, so both
 * branches are provable on any machine without one of the interpreters present.
 */
export function sidecarPython({ platform = process.platform, root = ROOT, exists = fs.existsSync } = {}) {
...
/**
 * Where the cloned-voice venv's interpreter lives, by platform. A venv puts it
 * under `Scripts` on Windows and `bin` everywhere else — the one difference that
 * would otherwise send Linux looking for a Windows path and reporting the clone
 * "not installed" when it was sitting right there.
 */
export function clonePython({ platform = process.platform, root = ROOT } = {}) {
  return platform === "win32"
    ? path.join(root, ".venv-clone", "Scripts", "python.exe")
    : path.join(root, ".venv-clone", "bin", "python");
}

/**
 * The setup command to name in a "you have not installed this yet" message, so a
 * Linux user is never told to run a `.ps1` they cannot. `tier` is appended, e.g.
 * "--clone" / "-Clone".
 */
export function setupCommand(tier = "", { platform = process.platform } = {}) {
  if (platform === "win32") return `setup-greg.ps1${tier ? ` ${tier}` : ""}`;
  const flag = tier ? ` ${tier.replace(/^-(?=[A-Z])/, "--").toLowerCase()}` : "";
  return `./setup-greg.sh${flag}`;
}
