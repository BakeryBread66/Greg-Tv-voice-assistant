  ...
  return {
    ...
    closed() {
      ...
      timer = setTimer(() => {
        ...
        if (open === 0 && !fired) {
          fired = true;
          // `false` means "not stopping after all" — kept running for a
          // paired phone. The watch re-arms, so the NEXT close still stops
          // him if that setting has been switched off in the meantime.
          if (onAllClosed?.() === false) fired = false;
        }
      }, graceMs);
    },
