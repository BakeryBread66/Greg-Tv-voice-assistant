...
export function setVisionEnabled(on) {
  ...
  if (want && !state.proven) {
    // Three ways to be unable to open the eyes, and they must not read alike.
    // Switched off in config is a DECISION and is reversible by editing a file;
    // a failed test is a fact about the model and is not. Reporting the first as
    // the second tells somebody their hardware cannot do a thing it can, which
    // is the kind of confident wrongness this file exists to prevent.
    let error;
    if (state.offInConfig) {
      error =
        "cannot turn eyes on: they are switched off in config.json — " +
        "set vision.enabled to true and restart Greg";
    } else if (state.checked) {
      error = `cannot turn eyes on: the model failed its eyesight test at startup — ${state.reason}`;
    } else {
      error = "cannot turn eyes on: the eyesight test has not run";
    }
    return { ok: false, enabled, error };
  }
...
export async function verifyVision(provider, config) {
  ...
  if (config.vision?.enabled === false) {
    // `offInConfig` is a third fact beside proven and enabled, and it exists so
    // the refusal below can tell the truth. Without it this state is
    // indistinguishable from a model that sat the eyesight test and failed —
    // same checked:true, same proven:false — and setVisionEnabled duly told
    // anyone who asked that "the model failed its eyesight test at startup",
    // about a test that never ran. The reason string tacked on the end did not
    // rescue it: the sentence leads with the false half.
    //
    // Distinguished by a flag rather than by matching the reason text, because a
    // refusal that depends on parsing its own prose breaks the moment somebody
    // rewords it.
    state = { checked: true, proven: false, offInConfig: true, reason: "switched off in config.json" };
    enabled = false;
    return visionStatus();
  }
