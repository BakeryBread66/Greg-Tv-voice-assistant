...
// The page's own modules, imported rather than copied: one definition of what a
// follow-up setting and a key combination mean, shared by the page that acts on
// them and the server that stores them. Both are DOM-free for exactly this.
import { FOLLOW_UP_MODES, followUpMode } from "../public/wake.js";
import { normaliseHotkey, hotkeyProblem } from "../public/hotkey.js";
...
// What Greg.exe last said about the push-to-talk key: which combination it
// tried to claim from Windows, and whether it got it. Null until it reports —
// which it never will when Greg was started some other way, and the dialog says
// so rather than implying a key that only works inside his window works
// everywhere.
let hotkeyStatus = null;

/** The key, and what Greg.exe said about it — for Greg.exe to poll. */
export function hotkeyState() {
  return { key: normaliseHotkey(config?.pushToTalk?.key), status: hotkeyStatus };
}

/** Greg.exe reporting whether Windows gave it the key. */
export function setHotkeyStatus({ key, claimed, problem } = {}) {
  hotkeyStatus = {
    key: normaliseHotkey(key),
    claimed: claimed === true,
    problem: typeof problem === "string" ? problem.slice(0, 300) : "",
    at: Date.now(),
  };
  announce();
  return hotkeyStatus;
...
export function settingsState() {
  ...
  return {
    ...
    listening: {
      // "always", "question" or "off". followUpEnabled is kept beside it for
      // anything that still asks the older yes-or-no question.
      followUpMode: followUpMode(followUp),
      followUpEnabled: followUpMode(followUp) !== "off",
    ...
    // One press and he is listening, no wake word. Empty means no key.
    pushToTalk: {
      key: normaliseHotkey(c.pushToTalk?.key),
      // Only a report about THIS key counts; one about the key before it is stale.
      global: hotkeyStatus && hotkeyStatus.key === normaliseHotkey(c.pushToTalk?.key) ? hotkeyStatus : null,
    },
  };
}
...
export async function applySettings(patch = {}) {
  ...
  if (patch.listening) {
    ...
    if (typeof l.followUpEnabled === "boolean") {
      config.followUp.enabled = l.followUpEnabled;
      // Switched on by something that only knows yes-or-no: on means on.
      if (l.followUpEnabled && config.followUp.mode === "off") config.followUp.mode = "always";
    }
    // The mode wins over the yes-or-no when both arrive, and "enabled" is kept
    // in step so a config read by an older Greg still means the same thing.
    if (FOLLOW_UP_MODES.includes(l.followUpMode)) {
      config.followUp.mode = l.followUpMode;
      config.followUp.enabled = l.followUpMode !== "off";
    }
  ...
  if (patch.pushToTalk && typeof patch.pushToTalk.key === "string") {
    const problem = hotkeyProblem(patch.pushToTalk.key);
    if (problem) problems.push(problem);
    else {
      config.pushToTalk ??= {};
      config.pushToTalk.key = normaliseHotkey(patch.pushToTalk.key);
    }
  }
 ...
 */
/**
 * Save the live config and tell every window, for a module that changes config
 * without going through applySettings — lib/claude-brain.js, whose switch has
 * to check a key over the network before it may change anything.
 */
