...
/**
 * The line of a Python traceback worth showing somebody.
 *
 * The sidecar's stderr was being reported by taking its last 200 characters,
 * which is the wrong end. A PyTorch CUDA failure ends with generic advice —
 * "For debugging consider passing CUDA_LAUNCH_BLOCKING=1" — while the sentence
 * that says what actually happened sits several lines above it. Reported by a
 * user who got exactly that boilerplate and nothing else, on a card that had
 * simply run out of memory.
 *
 * So: find the exception line, which is the one a traceback ends its *message*
 * on, and prefer it over whatever happens to be last.
 */
export function usefulError(stderr, { limit = 240 } = {}) {
  const lines = String(stderr ?? "")
    // Split on \r as well as \n: a tqdm progress bar redraws with carriage
    // returns, so the whole thing is ONE line and taking "the last line" hands
    // the user a wall of "Sampling: 3%|... 31.65it/s". Reported exactly that.
    .split(/[\r\n]+/)
    .map((l) => l.trim())
    // Progress bars are not errors. Neither is an empty redraw.
    .filter((l) => l && !/\d+%\|/.test(l) && !/\d+it\/s/.test(l));
  if (!lines.length) return "";

  // Running out of video memory is the common one and deserves an answer rather
  // than a diagnosis. Greg knows what else is on the card and what to do about
  // it; torch does not.
  if (/out of memory|CUDA error|CUBLAS_STATUS_ALLOC_FAILED/i.test(stderr)) {
    const asked = /Tried to allocate ([\d.]+ [KMG]iB)/i.exec(stderr);
    return (
      `the graphics card ran out of memory${asked ? ` (it needed ${asked[1]} more)` : ""}. ` +
      `The cloned voice wants about 5.4 GB on top of the brain. Turn his eyes off to free ~5.9 GB, ` +
      `or set clonedVoice.device to "cpu" in config.json to keep it off the card entirely`
    );
  }

  // Otherwise the exception line: "SomeError: what went wrong", last one wins,
  // since a chained traceback ends on the error that actually stopped it.
  const exception = [...lines].reverse().find((l) => /^[\w.]*(Error|Exception)\b/.test(l));
  return (exception ?? lines[lines.length - 1]).slice(0, limit);
}
...
export function currentVoice(config = {}) {
  // What is LOADED, not what config names — and the difference is a bug that
  // stranded somebody for a day.
  //
  // planVoiceChange short-circuits on "already using it". Reading the config
  // made that true the moment a reference was SAVED, which happens
  // unconditionally and before the sidecar is even asked to start. So once a
  // clone had failed to load — out of memory, in the reported case — picking
  // that same voice again returned change:false, applySettings never called the
  // switcher, and the dropdown silently did nothing forever. The one state where
  // a retry is the entire point is the one where it refused to try.
  //
  // `clonedVoice`, NOT `clone`. The first version of this read `config.clone` —
  // a key nothing writes and nothing reads — so a clone swap set a field into
  // space and reported "disabled in config". Guessed from the module name
  // instead of opening the file that consumes it.
  const cloneLive = cloneStatus().state === "ready";
  const piperLive = piperStatus().state === "ready";
  return {
    cloneReference: cloneLive ? config.clonedVoice?.reference ?? null : null,
    piperVoice: piperLive ? config.localVoice?.voice ?? null : null,
    // What config asks for, regardless of whether it worked. The dialog needs
    // this to show a selection; only the two above may decide "no change".
    wantedClone: config.clonedVoice?.reference ?? null,
    wantedPiper: config.localVoice?.voice ?? null,
  };
}

/**
 * The id of the voice actually loaded right now, or null.
 *
 * For the dialog, which was asking `currentVoice(config)?.id` — an object that
 * has no `id` — so the "(current)" marker never once appeared. Silent, because
 * `undefined ?? null` is a perfectly good "nothing is selected".
 *
 * Paths are compared normalised: config may hold "voices/x.wav" while
 * detectVoices builds "voices\x.wav" from path.join, and on Windows those are
 * the same file and different strings.
 */
export function loadedVoiceId(config, options) {
  ...
  const all = listVoices(options);
  if (now.cloneReference) {
    const hit = all.find((v) => v.kind === "clone" && same(v.file, now.cloneReference));
    if (hit) return hit.id;
  }
  if (now.piperVoice) {
    const hit = all.find((v) => v.kind === "piper" && v.id === now.piperVoice);
    if (hit) return hit.id;
  }
  return null;
}
