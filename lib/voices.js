 ...
 */
export function cloneAvailability(config, { cloneWanted = true, runtime = null, reason = "" } = {}) {
  ...
  if (runtime === "unavailable") {
    // Carries the sidecar's own reason, which names the actual missing piece —
    // no Python 3.12, no reference clip, no CUDA. Far more use than "failed".
    return { ready: false, fix: `The cloned voice could not start${reason ? `: ${reason}` : "."}` };
  }
