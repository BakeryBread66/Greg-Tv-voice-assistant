 ...
 *
 * States the level, the move and the session, and stops. No characterisation of
 * whether that is good, and nothing that could be read as advice — see the note
 * at the top of this file.
 */
export function describeStocks(data) {
  if (!data?.index) return "I couldn't get the market figures.";
  const { name, price, change, changePercent } = data.index;
  if (price === null) return `I couldn't get a level for the ${name}.`;

  const direction = change > 0 ? "up" : change < 0 ? "down" : "flat";
  const move =
    change === null || changePercent === null
      ? ""
      : ` ${direction}${direction === "flat" ? "" : ` ${Math.abs(changePercent).toFixed(2)} percent`}`;
  const when = data.session === "open" ? "" : ", at the last close";
  return `The ${name} is at ${Math.round(price).toLocaleString("en-US")}${move}${when}.`;
}
