  ...
  try {
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      if (!toolCalls.length) {
        ...
        return { reply: greeting ? `${greeting} ${body}` : body, usedTools, historyCleared, timing };
      }
    ...
    return { reply: "That turned into more of a rabbit hole than I expected. Try asking a different way?", usedTools, historyCleared, timing };
  } catch (err) {
    console.error("[brain]", err);
    // A wipe that happened before the failure still happened.
    if (historyCleared) history.length = 0;
    return { reply: `My brain hit a snag. ${err.message}`, usedTools, historyCleared, timing };
  }
}
