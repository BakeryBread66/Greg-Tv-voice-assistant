...
//
// Three tiers:
//   1. A local model via Ollama  — the default; no key, no account, works offline
//   2. Claude                    — only when config.json names it AND a key is set
//   3. Basic mode                — phrase matching, if no brain is available
//
...
import { searchConversations, wantsHistoryCleared } from "./conversation-log.js";
...
import { quickIntent, spokenTime, spokenDate, asksAboutScreen, SCREEN_FROM_PHONE } from "./quick.js";
...
export async function initBrain(config) {
  ...
  // Start from nothing. A second call that found no brain used to keep the
  // FIRST call's one — harmless at startup, where this runs once, and wrong
  // for anything that asks twice.
  provider = null;
  ...
  const candidates =
    preference === "ollama"
      ...
      : // "auto": the brain on this PC, and nothing else.
        //
        // It used to prefer Claude whenever a key existed, which made a key in
        // .env — put there for any reason — a switch that silently sent every
        // conversation to Anthropic. Worse, the choice is made ONCE, here: if
        // Ollama was still starting when Greg did, a key turned a slow boot into
        // a cloud session for the rest of the day, with nothing in the window to
        // say so. Leaving this machine is now something you ask for by name.
        [createOllamaProvider(config)];
  ...
  // A key that is present and NOT being used is worth one line, so nobody who
  // meant to use Claude is left wondering why Greg is on the local model — or,
  // with Ollama down, in basic mode.
  if (preference === "auto" && process.env.ANTHROPIC_API_KEY?.trim()) {
    console.log(
      `[brain] An ANTHROPIC_API_KEY is set but not used: "provider" is "auto", which never sends your ` +
      `conversation off this PC. Choose Claude in Settings, on the Brain tab, to use it.`
    );
  }
...
export function describeBrain() {
  if (!provider) return { active: false, label: "basic mode (no brain found)", onThisMachine: true, service: "this PC" };
  // A provider that does not say where it runs is treated as NOT on this PC.
  // The badge that reads this exists to disclose, and an unknown is the case
  // where hiding it would be the wrong way to be wrong.
  const onThisMachine = provider.where?.onThisMachine === true;
  return {
    active: true,
    ...
    toolsUsable,
    onThisMachine,
    service: provider.where?.service ?? "a server that is not this PC",
  };
}
...
/**
 * What a paired phone may not use: anything that sees this PC's screen or reads
 * its files. Enforced three times over, in code — the phone's turn is never
 * offered these tools, its prompt says it has none, and perform() refuses them
 * if a model calls one anyway. A prompt alone has lost that argument before.
 */
export const PHONE_BLOCKED = new Set(["look_at_screen", "take_screenshot", "read_file", "set_vision"]);

function toolsFor({ remote = false } = {}) {
  if (!toolsUsable) return [];
  const tools = visionStatus().ok ? TOOLS : TOOLS.filter((tool) => tool.name !== "look_at_screen");
  return remote ? tools.filter((tool) => !PHONE_BLOCKED.has(tool.name)) : tools;
}

// In place of the screen rule, when the question came from a phone.
const PHONE_RULE =
  "- You are talking to the user through their PHONE, away from their PC. From the phone you cannot see or capture the PC's screen and cannot open or read files on it: those tools do not exist in this conversation. If they ask for either, say plainly that it only works when they are at the PC. Never guess at what is on the screen or in a file.";
// ---------------------------------------------------------------------------
// Tools
//
 ...
 */
function toolRules({ remote = false } = {}) {
  if (toolsUsable) {
    const rules = `You have tools that fetch live data, and you MUST use them:
...
- Anything lasting the user tells you about themselves, or an explicit "remember that" -> call remember_about_user. If they ask you to forget a fact, call forget_about_user.
- Asked to clear, delete or wipe their conversation history or chat log -> call clear_conversation_history. forget_about_user only removes remembered facts and never touches the history; if they want both gone, call both.
...
Never say you lack access to this information, and never ask the user for their location — call the tool instead.`;
    // From the phone there is no screenshot tool, so its rule goes too: an
    // instruction about a tool he does not have is an invitation to pretend.
    return remote ? rules.split("\n").filter((line) => !line.startsWith("- Asked to take, grab, capture or save a screenshot")).join("\n") : rules;
  }
...
- You CANNOT check the time, the date, the weather, the news, the markets, or what is playing. You cannot set or cancel a timer or reminder, save or forget a fact, clear the conversation history, change the channel, take a screenshot, search the web or read a page.
...
export function buildSystemPrompt(config, place, userText = "", { remote = false } = {}) {
...
/**
 * Were they asking for the conversation log to be cleared, and did he not?
 *
 * Measured from his own log before this existed: asked three times to clear
 * the history, he called forget_about_user all three times — which removes
 * remembered facts and not a line of the log — and said the history was gone.
 * The prompt now separates the two, but a prompt instruction is the thing this
 * project has learned not to rely on for a claim like this one. The request is
 * the fixed, testable half, so the gate reads that rather than parsing what he
 * said back: asked and not called, he says so.
 *
 * Deliberately NOT exempted when he asked a clarifying question instead. The
 * sentence it adds is still true in that case, and a false "it's cleared" is
 * the failure that matters.
 */
export function historyClearClaimedNotDone(userText, usedTools) {
  if (!wantsHistoryCleared(userText)) return null;
  if (usedTools.includes("clear_conversation_history")) return null;
  return "To be clear: I have not deleted our conversation history, so the log is still saved. Ask me to clear the conversation history and I will.";
}

/**
 * One line saying where a turn's time went: "model 1.9s > get_weather 0.4s >
 * model 2.1s".
 *
 * The total alone could not say which part was slow. His log showed the first
 * weather question of a session taking 12 to 50 seconds against five for the
 * rest, and nothing in it could tell a cold model from a slow weather service.
 * Now the console and the log both carry the breakdown.
 */
export function describeTiming(steps = []) {
  if (!Array.isArray(steps) || !steps.length) return "";
  return steps.map(([name, ms]) => `${name} ${(Math.max(0, Number(ms) || 0) / 1000).toFixed(1)}s`).join(" > ");
}

export async function think(userText, history, config, onSentence = null, awaySeconds = 0, { remote = false } = {}) {
  if (!resolved) await initBrain(config);
  if (!provider) {
    const reply = await fallbackReply(userText, config);
    onSentence?.(reply);
    return { reply, usedTools: [], timing: [] };
  }
  ...
  // A preface, like the filler below: said before the answer, and marked so the
  // page does not take it for the answer's first sentence - which it speaks
  // alone and at once, holding the rest to synthesise as one call.
  if (greeting) onSentence?.(greeting, { preface: true });

  // The time and the date are answered here, with no model at all — see
  // lib/quick.js for why. Still recorded as a get_current_time call, so the log
  // and "how do you know that?" both say what really happened.
  // From the phone, a question about the PC's screen gets a fixed answer, not a
  // model: see asksAboutScreen in lib/quick.js for the answer it gave instead.
  if (remote && asksAboutScreen(userText)) {
    onSentence?.(SCREEN_FROM_PHONE);
    const turn = [...history, { role: "user", content: userText }, { role: "assistant", content: SCREEN_FROM_PHONE }];
    history.length = 0;
    history.push(...trimHistory(forHistory(turn)));
    return { reply: greeting ? `${greeting} ${SCREEN_FROM_PHONE}` : SCREEN_FROM_PHONE, usedTools: [], historyCleared: false, timing: [] };
  }

  const quick = quickIntent(userText);
  if (quick === "time" || quick === "date") {
    const started = Date.now();
    const now = new Date();
    const clock = await runTool("get_current_time", {}, { config, provider, resolvePlace, userText });
    const body = quick === "time" ? spokenTime(now) : spokenDate(now);
    onSentence?.(body);
    const turn = [...history, { role: "user", content: userText }, { role: "assistant", content: body }];
    history.length = 0;
    history.push(...trimHistory(forHistory(turn)));
    recordProvenance({
      question: userText,
      reply: body,
      steps: [{ tool: "get_current_time", detail: summarise("get_current_time", {}, clock) }],
    });
    return {
      reply: greeting ? `${greeting} ${body}` : body,
      usedTools: ["get_current_time"],
      historyCleared: false,
      timing: [["get_current_time", Date.now() - started]],
    };
  }

  const loc = await getLocation(config);
  const place = [loc.city, loc.region].filter(Boolean).join(", ") || "an unknown location";
  const system = buildSystemPrompt(config, place, userText, { remote });

  const messages = [...history, { role: "user", content: userText }];
  const usedTools = [];
  // Set only when the log really was wiped this turn. The in-memory history is
  // dropped with it, or "clear my history" would leave the whole conversation
  // sitting in his context, one follow-up away from being repeated back.
  let historyCleared = false;
  // Where the time went, in order: ["model", ms] for each round and
  // [toolName, ms] for each call. See describeTiming.
  const timing = [];
  // "Let me look that up." - said while he works, when the work is slow. Only
  // when someone is listening for sentences; see lib/filler.js.
  const filler = createFiller({ say: onSentence ? (text) => onSentence(text, { preface: true }) : null });
  // The model's own sentences stop the filler's clock before they are spoken:
  // once the answer has started there is nothing to fill.
  const answerSentence = onSentence
    ? (sentence) => {
        filler.answering();
        onSentence(sentence);
      }
    : null;
  // What he said before the answer belongs in the transcript too, or the log
  // and /api/chat would disagree with what was said out loud.
  const withPrefaces = (body) => {
    const said = [greeting, filler.said].filter(Boolean).join(" ");
    return said ? `${said} ${body}` : body;
  };
  ...
  // Run one tool call and put its result where the model will read it. Used for
  // the calls the model makes and for the weather fetched before it is asked.
  const perform = async (call) => {
    // The third of the phone's three gates: whatever the model asked for, a
    // blocked tool does not run. Not counted in usedTools — nothing was done.
    if (remote && PHONE_BLOCKED.has(call.name)) {
      console.warn(`[brain] PHONE GATE — refused ${call.name} for a question from the phone`);
      timing.push([call.name, 0]);
      messages.push({
        role: "tool",
        toolCallId: call.id,
        name: call.name,
        content: JSON.stringify({
          error: "NOT AVAILABLE FROM THE PHONE. Nothing was looked at or read.",
          tell_the_user: "I can't see your screen or open your files from your phone — that only works when you're at the PC.",
        }),
      });
      return;
    }
    ...
    try {
      // userText rides along for the one tool that needs the user's own
      // words as permission: clear_conversation_history.
      const value = await runTool(call.name, call.args ?? {}, { config, provider, resolvePlace, userText });
      if (call.name === "clear_conversation_history" && value?.cleared === true) historyCleared = true;
    ...
    timing.push([call.name, Date.now() - toolStarted]);
    messages.push({ role: "tool", toolCallId: call.id, name: call.name, content: result });
  };

  try {
    // "What's the weather" is fetched before the model is asked, so its first
    // round phrases a forecast instead of spending a round deciding to fetch one.
    // Written into the conversation exactly as if the model had called it, so
    // nothing downstream can tell the difference. See lib/quick.js.
    if (quick === "weather") {
      const call = { id: "quick_weather", name: "get_weather", args: {} };
      messages.push({ role: "assistant", content: "", toolCalls: [call] });
      filler.toolsStarting([call.name], false);
      await perform(call);
    }

    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      const roundStarted = Date.now();
      const { text, toolCalls, spoke } = await runRound({ system, messages, onSentence: answerSentence, remote });
      timing.push(["model", Date.now() - roundStarted]);
      ...
      if (!toolCalls.length) {
        ...
        const notCleared = historyClearClaimedNotDone(userText, usedTools);
        if (notCleared) {
          console.warn(`[brain] HONESTY GUARD — asked to clear the history and clear_conversation_history was not called: "${userText}"`);
          onSentence?.(notCleared);
          body = appendSentence(body, notCleared);
        }
        ...
        return { reply: withPrefaces(body), usedTools, historyCleared, timing };
      }

      messages.push({ role: "assistant", content: text, toolCalls });
      filler.toolsStarting(toolCalls.map((call) => call.name), Boolean(spoke));

      for (const call of toolCalls) await perform(call);
    }

    if (historyCleared) history.length = 0;
    return { reply: withPrefaces("That turned into more of a rabbit hole than I expected. Try asking a different way?"), usedTools, historyCleared, timing };
  } catch (err) {
    console.error("[brain]", err);
    // A wipe that happened before the failure still happened.
    if (historyCleared) history.length = 0;
    return { reply: withPrefaces(`My brain hit a snag. ${err.message}`), usedTools, historyCleared, timing };
  } finally {
    // However the turn ended, a filler still counting down must not speak after it.
    filler.done();
  }
}
 ...
 */
async function runRound({ system, messages, onSentence, remote = false }) {
  if (!onSentence || typeof provider.completeStream !== "function") {
    const result = await provider.complete({ system, messages, tools: toolsFor({ remote }) });
  ...
  const result = await provider.completeStream({ system, messages, tools: toolsFor({ remote }) }, (delta) => {
  ...
  if (result.toolCalls.length) {
    ...
    // `spoke` tells the filler he has already said something this round, so it
    // does not say a second thing on top of it.
    return { ...result, spoke: spoken > 0 }; // the leftover buffer belongs to a tool turn; don't speak it
  }
