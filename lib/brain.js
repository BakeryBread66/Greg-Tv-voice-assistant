...
export async function initBrain(config) {
  ...
  if (preference === "auto" && process.env.ANTHROPIC_API_KEY?.trim()) {
    console.log(
      ...
      `conversation off this PC. Set "provider": "anthropic" in config.json to use Claude.`
    );
  }
...
export async function think(userText, history, config, onSentence = null, awaySeconds = 0) {
  ...
  try {
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      const { text, toolCalls, spoke } = await runRound({ system, messages, onSentence: answerSentence });
