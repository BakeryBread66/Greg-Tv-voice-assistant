...
/**
 * What is being asked for, from the three fields the model filled in.
 *
 * "Play some Prince" failed with "no song name was given", and not because of
 * Prince: the schema tells the model to put an artist in its own field, and when
 * that is ALL someone named, the title comes through empty. Every artist asked
 * for by name alone failed the same way. An artist and no title is a request
 * for the artist.
 */
export function planPlay(query, kind = "track", artistHint = "") {
  const type = ["track", "album", "artist", "playlist"].includes(kind) ? kind : "track";
  const { title, artist } = splitTitleAndArtist(query, artistHint);
  if (!title && artist) return { type: "artist", title: artist, artist: "" };
  // An artist search is on the name alone; there is no second field to fill.
  if (type === "artist") return { type, title: title || artist, artist: "" };
  return { type, title, artist };
}

/**
 * The artist to play instead, when a bare name asked for as a song is a band.
 *
 * The model's other way of hearing "put on Steely Dan" is a TRACK called
 * "Steely Dan" — which plays whatever song has those words in its title. So when
 * no artist came with the request, the name is checked against artists too, and
 * an artist called exactly that wins unless a song called exactly that is at
 * least as popular. "Hello" stays Adele's song; "Adele" becomes Adele.
 *
 * Exact names only. "Prince" must not become "Princess Nokia" because one
 * starts with the other.
 */
export function artistInstead(tracks, artists, title) {
  const wanted = normalise(title);
  if (!wanted) return null;
  const artist = (artists ?? []).find((item) => normalise(item?.name) === wanted);
  if (!artist) return null;
  const track = (tracks ?? []).find((item) => normalise(item?.name) === wanted);
  if (!track) return artist;
  return (artist.popularity ?? 0) >= (track.popularity ?? 0) ? artist : null;
}
...
export async function playSomething(query, kind = "track", artistHint = "") {
  const plan = planPlay(query, kind, artistHint);
  const { title, artist } = plan;
  let type = plan.type; // becomes "artist" if the name turns out to be a band
  if (!title) return { error: "NOTHING IS PLAYING. No song, album or artist was named." };
  ...
  let item = pickBest(items, title);

  // A bare name asked for as a song may be a band. One more search, only when
  // no artist came with the request — "Butterfly Effect by Travis Scott" is
  // plainly a song and never pays for it.
  if (type === "track" && !artist) {
    let artists = [];
    try {
      const found = await call(`/search?q=${encodeURIComponent(buildQuery("artist", title, ""))}&type=artist&limit=10`);
      artists = (found?.artists?.items ?? []).filter(Boolean);
    } catch {
      artists = []; // the song search already answered; don't lose it over this
    }
    const band = artistInstead(items, artists, title);
    if (band) {
      item = band;
      type = "artist";
    }
