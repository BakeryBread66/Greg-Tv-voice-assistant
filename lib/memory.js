...
// --- The Settings dialog ------------------------------------------------------
//
// The user seeing, and correcting, what he believes about them. Found by EXACT
// text rather than by the word matching forget() does: a click on one fact must
// never remove another that happens to share its words.

/** Every fact, with when it was saved. */
export function listFactRecords() {
  return load().facts.map((f) => ({ text: f.text, savedAt: f.savedAt ?? null }));
}

/** Reword one fact. */
export function editFact(was, text) {
  const fact = String(text ?? "").trim().replace(/\s+/g, " ");
  if (!fact) return { error: "A fact can't be empty. Delete it instead." };
  if (fact.length > 300) return { error: "That's too long to remember as a single fact." };
  const data = load();
  const index = data.facts.findIndex((f) => f.text === was);
  if (index === -1) return { error: "That fact isn't there any more." };
  if (data.facts.some((f, i) => i !== index && f.text === fact)) return { error: "He already knows exactly that." };
  data.facts[index] = { text: fact, savedAt: new Date().toISOString() };
  save();
  return { ok: true, text: fact };
}

/** Forget exactly one fact. */
export function deleteFact(text) {
  const data = load();
  const index = data.facts.findIndex((f) => f.text === text);
  if (index === -1) return { error: "That fact isn't there any more." };
