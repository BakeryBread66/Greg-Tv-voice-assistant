 ...
 */
export function logTurn({ user, reply, usedTools = [], ms = null, firstMs = null, answerMs = null, timing = null, via = null }) {
  ...
  const line = JSON.stringify({
    ...
    // Which paired phone asked, by the name it was paired under. Absent for
    // the PC's own window.
    ...(via ? { via: String(via).slice(0, 40) } : {}),
  });
  try {
