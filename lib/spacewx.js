...
export function describeSpaceWeather(reading) {
  if (!reading) return "I couldn't reach NOAA's space weather feed.";
  const storm = reading.kpWhole >= 5 ? ` That is a ${reading.scale} ${reading.condition.toLowerCase()} storm.` : "";
  const aurora = reading.aurora?.visible === true ? ` ${reading.aurora.text}` : "";
  return `The planetary K index is ${reading.kp.toFixed(1)}, which is ${reading.condition.toLowerCase()} and ${reading.trend}.${storm}${aurora}`;
}
