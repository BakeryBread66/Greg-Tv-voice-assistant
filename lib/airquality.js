 ...
 */
// Reused for fifteen minutes, which is how often the service itself updates.
// The air rides along on every weather answer, so without this it was a second
// network round trip on the commonest question Greg is asked. The promise is
// kept rather than the result, so concurrent asks share one request, and a
// failure is dropped so the next ask tries again.
export const AIR_TTL_MS = 15 * 60 * 1000;
const readings = new Map(); // "lat,lon" -> { at, promise }

export function clearAirCache() {
  readings.clear();
}

function fetchAir(url, key, now = Date.now()) {
  const hit = readings.get(key);
  if (hit && now - hit.at < AIR_TTL_MS) return hit.promise;
  const promise = (async () => {
    const res = await fetch(url, { signal: AbortSignal.timeout(10000) });
    if (!res.ok) throw new Error(`air quality service returned ${res.status}`);
    return res.json();
  })();
  readings.set(key, { at: now, promise });
  promise.catch(() => {
    if (readings.get(key)?.promise === promise) readings.delete(key);
  });
  return promise;
}
...
export async function getAirQuality(config = {}, place = null) {
  ...
  const data = await fetchAir(url, `${latitude.toFixed(3)},${longitude.toFixed(3)}`);
